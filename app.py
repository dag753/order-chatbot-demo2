...
from ui_components import render_sidebar, display_chat_messages # Import new UI functions
...
async def process_message(workflow, user_query):
    ...
    try:
        ...
        return ChatResponseStopEvent(
            result=None,
            response=f"I'm sorry, I couldn't process your request. Error in workflow execution: {str(e)}",
            action_type="error"
        )
    ...
    try:
        if response_event is None or not isinstance(response_event, ChatResponseStopEvent):
            ...
            if not initial_message_content or initial_message_content.strip() in ['{', '}', '[]', '[', ']', '{}']:
                ...
                logger.warning(f"Found empty or invalid fragment in initial response: {response_event.response}")
            ...
            with st.spinner("Getting details..."):
                ...
                try:
                    ...
                    if action_type == "menu_inquiry_pending":
                        ...
                        detailed_response_content = await chat_workflow._handle_order_query(original_prompt) # Changed to await
...
def main():
    ...
    render_sidebar(st.session_state.menu, st.session_state.actions)
